...
if npks > 1 
    ...
    for i=1:nmx
        ...
        roi=tmp( max(1,(mx(i,1)-floor(sz/2))):min(size(tmp,1),(mx(i,1)+(floor(sz/2)+1))),max(1,(mx(i,2)-floor(sz/2))):min(size(tmp,2),(mx(i,2)+(floor(sz/2)+1)))) ;
        [mv,indi]=max(roi);
        [mv,indj]=max(mv);
        tmp( max(1,(mx(i,1)-floor(sz/2))):min(size(tmp,1),(mx(i,1)+(floor(sz/2)+1))),max(1,(mx(i,2)-floor(sz/2))):min(size(tmp,2),(mx(i,2)+(floor(sz/2)+1))))=0;
        tmp(max(1,(mx(i,1)-floor(sz/2)))+indi(indj)-1,max(1,(mx(i,2)-floor(sz/2)))+indj-1)=mv;
    end
    ind=find(tmp>0);
